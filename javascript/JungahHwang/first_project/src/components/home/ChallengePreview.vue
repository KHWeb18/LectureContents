<template>
  <v-card class="pa-3" color="primary">
    <v-card class="pa-3" max-height="100%">
      <v-tooltip top>
        <template v-slot:activator="{on}">
          <p class="headline ma-3" v-on="on" @click="readChallenge">Challenge</p>
        </template>
        <span>이동하기</span>
      </v-tooltip>
...
export default {
  ...
  mounted () {
    for (let i = 0; i < 3; i++) {
      let list = thankyouBubu.items
      
      this.videoList.push(
        { videoId: list[i].id.videoId, 
          thumbnail: list[i].snippet.thumbnails.high.url, 
